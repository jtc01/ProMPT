...
class UnclearTaskEditor(BaseAgent):
    """
    Editor for unclear task prompts - makes minimal changes for improvement
    """
    def __init__(self, model="llama3.2"):
        super().__init__(model)
        
        self.prompt_template = ChatPromptTemplate.from_template("""
        You are an expert prompt editor specializing in unclear or ambiguous tasks.
        ...
        the feedback while maintaining the inferred intent. Unlike a prompt generator, you should 
        NOT rewrite the entire prompt - make targeted edits to fix specific issues.
        
        Focus particularly on:
        1. Maintaining a plausible inference about user intent
        2. Addressing the specific weaknesses identified in the review
